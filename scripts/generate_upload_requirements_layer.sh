...
layers_bucket=$(echo ${DDS_TERRAFORM_BACKEND_BUCKET_TEMPLATE} | sed 's/{account_id}/'"${account_id}"'/')
...
build_path="$DDS_HOME/dependencies/python/lib/python3.6/site-packages"
dependency_dir="$DDS_HOME/dependencies"
aws_req_key=dds/$DDS_DEPLOYMENT_STAGE/requirements.txt
aws_zip_key=dds/$DDS_DEPLOYMENT_STAGE/dds-dependencies-$DDS_DEPLOYMENT_STAGE.zip
local_temp_chalice=${DDS_HOME}/temp_domovoi
local_req=$DDS_HOME/requirements.txt
local_zip=$DDS_HOME/dds-dependencies-$DDS_DEPLOYMENT_STAGE.zip
layer_name=dds-dependencies-${DDS_DEPLOYMENT_STAGE}
...
function verify_chalice_folder() {
	if [[ ! -d $1/domovoilib/dds ]]
	then
		cp -R ${DDS_HOME}/dds $1/chalicelib
	fi
...
function build_chalice() {
    ...
    # copy chalice folder
    cp -R ${DDS_HOME}/daemons/dds-delete/ ${local_temp_chalice}
    verify_chalice_folder ${local_temp_chalice}
    mkdir ${local_temp_chalice}/vendor
    cp -R ${DDS_HOME}/vendor.in ${local_temp_chalice}; ln -s ${DDS_HOME}/requirements.txt ${local_temp_chalice}/requirements.txt;
    cd ${local_temp_chalice}
    for wheel in vendor.in/*/*.whl; do unzip -q -o -d vendor $wheel; done
    chalice package .
    echo "removing non-dependencies from zip"
    # chalice does not package the domovoi lib, or any of the TF files/ folders
...
function upload() {
    ...
    echo "Uploading zip to ${layers_bucket} for stage: ${DDS_DEPLOYMENT_STAGE}"
